...
class crodnet:
    ...
    def net(self, inputs, reuse=tf.AUTO_REUSE, dropout_keep_prob=0.5):
        with tf.variable_scope('vgg_16'):
            net = slim.repeat(inputs, 2, slim.conv2d, 64, [3, 3], scope='conv1', padding='same', reuse=reuse)  # 352
            print('conv1: ' + str(net))
            net = slim.max_pool2d(net, [2, 2], scope='pool1')
            print('pool1: ' + str(net))
            net = slim.repeat(net, 2, slim.conv2d, 128, [3, 3], scope='conv2', padding='same', reuse=reuse)  # 176
            print('conv2: ' + str(net))
            net = slim.max_pool2d(net, [2, 2], scope='pool2')
            print('pool2: ' + str(net))
            net = slim.repeat(net, 3, slim.conv2d, 256, [3, 3], scope='conv3', padding='same', reuse=reuse)  # 88
            print('conv3: ' + str(net))
            net = slim.max_pool2d(net, [2, 2], scope='pool3')
            print('pool3: ' + str(net))
            net = slim.repeat(net, 3, slim.conv2d, 512, [3, 3], scope='conv4', padding='same', reuse=reuse)  # 44
            print('conv4: ' + str(net))
            net = slim.max_pool2d(net, [2, 2], scope='pool4')
            print('pool4: ' + str(net))
            net = slim.repeat(net, 3, slim.conv2d, 512, [3, 3], scope='conv5', padding='same', reuse=reuse)  # 22
            print('conv5: ' + str(net))
            net = slim.max_pool2d(net, [2, 2], stride=1, scope='pool5', padding='same')
            print('pool5: ' + str(net))

            net = slim.conv2d(net, 1024, [3, 3], rate=6, scope='fc6_sub', reuse=reuse)
            net = slim.conv2d(net, 1024, [1, 1], scope='fc7_sub', reuse=reuse)

            with tf.variable_scope('conv6_2'):
                net = slim.conv2d(net, 256, [1, 1], scope='conv1x1', reuse=reuse)
                print('conv6/conv1x1: ' + str(net))
                net = tf.pad(net, tf.constant([[0, 0], [1, 1], [1, 1], [0, 0]]), "CONSTANT")
                print('conv6/pad: ' + str(net))
                net = slim.conv2d(net, 512, [3, 3], stride=2, scope='conv3x3', padding='VALID', reuse=reuse)  # 11
                print('conv6/conv3x3: ' + str(net))

            with tf.variable_scope('conv7_2'):
                net = slim.conv2d(net, 128, [1, 1], scope='conv1x1', reuse=reuse)
                print('conv7/conv1x1: ' + str(net))
                net = tf.pad(net, tf.constant([[0, 0], [1, 1], [1, 1], [0, 0]]), "CONSTANT")
                print('conv7/pad: ' + str(net))
                net = slim.conv2d(net, 256, [3, 3], stride=1, scope='conv3x3', padding='VALID', reuse=reuse)
            ...
            with tf.variable_scope('conv8_2'):
                net = slim.conv2d(net, 128, [1, 1], scope='conv1x1', reuse=reuse)
                net = slim.conv2d(net, 256, [3, 3], scope='conv3x3', padding='VALID', reuse=reuse)  # 9

            with tf.variable_scope('conv9_2'):
                net = slim.conv2d(net, 128, [1, 1], scope='conv1x1', reuse=reuse)
                net = slim.conv2d(net, 256, [3, 3], scope='conv3x3', padding='VALID', reuse=reuse)  # 7

            # net = slim.max_pool2d(net, [2, 2], stride=1, scope='pool7', padding='same')

            # net = slim.conv2d(net, 256, [3, 3], scope='conv10', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())  # 3
            # net = slim.conv2d(net, 128, [3, 3], scope='conv11', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())  # 1

            # with tf.variable_scope('branch1'):
            #     branch1 = slim.conv2d(net, 128, [1, 1], scope='conv1', reuse=reuse, weights_initializer=tf.initializers.he_normal())
            #     branch1 = slim.conv2d(branch1, 128, [7, 1], scope='conv2', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())
            #     branch1 = slim.conv2d(branch1, 128, [1, 7], scope='conv3', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())
            #     branch1 = l2_normalization(branch1)
            #
            # with tf.variable_scope('branch2'):
            #     branch2 = slim.max_pool2d(net, [3, 3], stride=1, scope='pool', padding='same')
            #     branch2 = slim.conv2d(branch2, 64, [3, 3], scope='conv1', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())
            #     branch2 = slim.conv2d(branch2, 64, [3, 3], scope='conv2', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())
            #     branch2 = slim.conv2d(branch2, 64, [3, 3], scope='conv3', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())
            #     branch2 = l2_normalization(branch2)
            #
            # with tf.variable_scope('branch3'):
            #     branch3 = slim.conv2d(net, 128, [3, 3], rate=2, scope='conv1', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())  # 3
            #     branch3 = slim.conv2d(branch3, 128, [3, 1], scope='conv2', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())
            #     branch3 = slim.conv2d(branch3, 128, [1, 3], scope='conv3', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())
            #     branch3 = l2_normalization(branch3)
            #
            # net = tf.concat([branch1, branch2, branch3], axis=-1)

            # net = slim.conv2d(net, 256, [7, 1], scope='conv10', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())
            # net = slim.conv2d(net, 256, [1, 7], scope='conv11', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())

            net = slim.conv2d(net, 128, [3, 1], rate=2, scope='conv10', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())
            net = slim.conv2d(net, 128, [1, 3], rate=2, scope='conv11', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())
            net = slim.conv2d(net, 128, [3, 1], scope='conv12', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())
            net = slim.conv2d(net, 128, [1, 3], scope='conv13', padding='valid', reuse=reuse, weights_initializer=tf.initializers.he_normal())

            net = slim.conv2d(net, self.opts.lcr, [1, 1], scope='conv_last', reuse=reuse, weights_initializer=tf.initializers.he_normal())

            common_representation = net

            n_channels_final = self.get_last_layer_n_channels()
            with tf.variable_scope('localization'):
                net = slim.conv2d(net, self.opts.lcr, [1, 1], scope='layer1', reuse=reuse, weights_initializer=tf.initializers.he_normal())
                net = slim.conv2d(net, n_channels_final, [1, 1], activation_fn=None, normalizer_fn=None, scope='layer2', reuse=reuse, weights_initializer=tf.initializers.he_normal())
            return net, common_representation
    ...
    def write_input_crops(self, inputs_all_sizes, labels, filenames, all_outputs):
        ...
        net_output_conf, net_output_coords, net_output_pc = self.split_net_output_np(all_outputs)
        ...
        if self.opts.box_per_class:
            ...
            selected_coords = self.get_selected_coords_np(net_output_conf, net_output_coords)  # (batch_size, nboxes, 4)
    ...
    def get_last_layer_n_channels(self):
        if self.opts.predict_coordinates:
            if self.opts.box_per_class:
                if self.opts.predict_pc:
                    n_channels_final = self.nclasses * 6  # For each class, a confidence, four coordinates and pc.
                    # [cl1_conf, cl1_dcx_enc, cl1_dcy_enc, cl1_w_enc, cl1_h_enc, cl1_pc, cl2...]
                else:
                    n_channels_final = self.nclasses * 5  # For each class, a confidence and four coordinates.
                    # [cl1_conf, cl1_dcx_enc, cl1_dcy_enc, cl1_w_enc, cl1_h_enc, cl2...]
            else:
                if self.opts.predict_pc:
                    n_channels_final = self.nclasses + 5  # A confidence for each class, plus four coordinates and pc.
                    # [cl1_conf, cl2_conf, ..., dcx_enc, dcy_enc, w_enc, h_enc, pc]
                else:
                    n_channels_final = self.nclasses + 4  # A confidence for each class, plus four coordinates.
                    # [cl1_conf, cl2_conf, ..., dcx_enc, dcy_enc, w_enc, h_enc]
        else:
            n_channels_final = self.nclasses  # A confidence for each class.
            # [cl1_conf, cl2_conf, ...]
            if self.opts.predict_pc:
                raise Exception('Option predict_pc not available when not predicting coordinates.')
        return n_channels_final

    def split_net_output_np(self, net_output):
        # net_output: (..., ?)
        if self.opts.predict_coordinates:
            if self.opts.box_per_class:
                if self.opts.predict_pc:
                    ...
                    # [cl1_conf, cl1_dcx_enc, cl1_dcy_enc, cl1_w_enc, cl1_h_enc, cl1_pc, cl2...]
                    net_output_conf = net_output[..., 0:(self.nclasses*6):6]
                    dcx_enc = net_output[..., 1:(self.nclasses*6):6]  # (..., n_classes)
                    dcy_enc = net_output[..., 2:(self.nclasses*6):6]  # (..., n_classes)
                    w_enc = net_output[..., 3:(self.nclasses*6):6]  # (..., n_classes)
                    h_enc = net_output[..., 4:(self.nclasses*6):6]  # (..., n_classes)
                    net_output_coords = np.concatenate([dcx_enc, dcy_enc, w_enc, h_enc], axis=-1)  # (..., 4 * n_classes)
                    net_output_pc = net_output[..., 5:(self.nclasses*6):6]  # (..., n_classes)
                else:
                    # net_output: (..., 5 * nclasses)
                    # [cl1_conf, cl1_dcx_enc, cl1_dcy_enc, cl1_w_enc, cl1_h_enc, cl2...]
                    net_output_conf = net_output[:, :, 0:(self.nclasses*5):5]
                    dcx_enc = net_output[..., 1:(self.nclasses*5):5]  # (..., n_classes)
                    dcy_enc = net_output[..., 2:(self.nclasses*5):5]  # (..., n_classes)
                    w_enc = net_output[..., 3:(self.nclasses*5):5]  # (..., n_classes)
                    h_enc = net_output[..., 4:(self.nclasses*5):5]  # (..., n_classes)
                    net_output_coords = np.concatenate([dcx_enc, dcy_enc, w_enc, h_enc], axis=-1)  # (..., 4 * n_classes)
                    net_output_pc = None
            else:
                if self.opts.predict_pc:
                    ...
                    # [cl1_conf, cl2_conf, ..., dcx_enc, dcy_enc, w_enc, h_enc, pc]
                    net_output_conf = net_output[..., :self.nclasses]
                    net_output_coords = net_output[..., self.nclasses:(self.nclasses+4)]  # (..., 4)
                    net_output_pc = net_output[..., (self.nclasses+4)]  # (...)
                else:
                    # net_output: (..., nclasses + 4)
                    # [cl1_conf, cl2_conf, ..., dcx_enc, dcy_enc, w_enc, h_enc]
                    net_output_conf = net_output[..., :self.nclasses]
                    net_output_coords = net_output[..., self.nclasses:]  # (..., 4)
                    net_output_pc = None
        else:
            # net_output: (..., nclasses)
            # [cl1_conf, cl2_conf, ...]
    ...
    def split_net_output_tf(self, net_output):
        if self.opts.predict_coordinates:
            if self.opts.box_per_class:
                if self.opts.predict_pc:
                    # [cl1_conf, cl1_dcx_enc, cl1_dcy_enc, cl1_w_enc, cl1_h_enc, cl1_pc, cl2...]
                    net_output_conf = net_output[:, :, 0:(self.nclasses*6):6]
                    dcx_enc = net_output[:, :, 1:(self.nclasses*6):6]  # (batch_size, n_boxes, n_classes)
                    dcy_enc = net_output[:, :, 2:(self.nclasses*6):6]  # (batch_size, n_boxes, n_classes)
                    w_enc = net_output[:, :, 3:(self.nclasses*6):6]  # (batch_size, n_boxes, n_classes)
                    h_enc = net_output[:, :, 4:(self.nclasses*6):6]  # (batch_size, n_boxes, n_classes)
                    net_output_coords = tf.concat([dcx_enc, dcy_enc, w_enc, h_enc], axis=-1)  # (batch_size, n_boxes, 4 * n_classes)
                    net_output_pc = net_output[:, :, 5:(self.nclasses*6):6]  # (batch_size, n_boxes, n_classes)
                else:
                    # [cl1_conf, cl1_dcx_enc, cl1_dcy_enc, cl1_w_enc, cl1_h_enc, cl2...]
                    net_output_conf = net_output[:, :, 0:(self.nclasses*5):5]
                    dcx_enc = net_output[:, :, 1:(self.nclasses*5):5]  # (batch_size, n_boxes, n_classes)
                    dcy_enc = net_output[:, :, 2:(self.nclasses*5):5]  # (batch_size, n_boxes, n_classes)
                    w_enc = net_output[:, :, 3:(self.nclasses*5):5]  # (batch_size, n_boxes, n_classes)
                    h_enc = net_output[:, :, 4:(self.nclasses*5):5]  # (batch_size, n_boxes, n_classes)
                    net_output_coords = tf.concat([dcx_enc, dcy_enc, w_enc, h_enc], axis=-1)  # (batch_size, n_boxes, 4 * n_classes)
                    net_output_pc = None
            else:
                if self.opts.predict_pc:
                    # [cl1_conf, cl2_conf, ..., dcx_enc, dcy_enc, w_enc, h_enc, pc]
                    net_output_conf = net_output[:, :, :self.nclasses]
                    net_output_coords = net_output[:, :, self.nclasses:(self.nclasses+4)]  # (batch_size, n_boxes, 4)
                    net_output_pc = net_output[:, :, (self.nclasses+4)]  # (batch_size, n_boxes)
                else:
                    # [cl1_conf, cl2_conf, ..., dcx_enc, dcy_enc, w_enc, h_enc]
                    net_output_conf = net_output[:, :, :self.nclasses]
                    net_output_coords = net_output[:, :, self.nclasses:]  # (batch_size, n_boxes, 4)
                    net_output_pc = None
        else:
            # [cl1_conf, cl2_conf, ...]
            net_output_conf = net_output[:, :, :self.nclasses]
    ...
    def put_together_net_output(self, net_output_conf, net_output_coords, net_output_pc):
        # net_output_conf: (batch_size, nboxes, nclasses)
        # net_output_coords: (batch_size, nboxes, ?)
        # net_output_pc: (batch_size, nboxes, ?)
        ...
        if self.opts.predict_coordinates:
            if self.opts.box_per_class:
                if self.opts.predict_pc:
                    # [cl1_conf, cl1_dcx_enc, cl1_dcy_enc, cl1_w_enc, cl1_h_enc, cl1_pc, cl2...]
                    concatenated = tf.stack([net_output_conf, net_output_coords[:, :, 0:(self.nclasses*4):4],
                                              net_output_coords[:, :, 1:(self.nclasses*4):4],
                                              net_output_coords[:, :, 2:(self.nclasses*4):4],
                                              net_output_coords[:, :, 3:(self.nclasses*4):4],
                                              net_output_pc], axis=-1)  # (batch_size, n_boxes, n_classes, 6)
                    net_output = tf.reshape(concatenated, shape=(batch_size, self.n_boxes, self.nclasses * 6))
                else:
                    # [cl1_conf, cl1_dcx_enc, cl1_dcy_enc, cl1_w_enc, cl1_h_enc, cl2...]
                    concatenated = tf.stack([net_output_conf, net_output_coords[:, :, 0:(self.nclasses*4):4],
                                              net_output_coords[:, :, 1:(self.nclasses*4):4],
                                              net_output_coords[:, :, 2:(self.nclasses*4):4],
                                              net_output_coords[:, :, 3:(self.nclasses*4):4]], axis=-1)  # (batch_size, n_boxes, n_classes, 5)
                    net_output = tf.reshape(concatenated, shape=(batch_size, self.n_boxes, self.nclasses * 5))
            else:
                if self.opts.predict_pc:
                    # [cl1_conf, cl2_conf, ..., dcx_enc, dcy_enc, w_enc, h_enc, pc]
                    net_output = tf.concat([net_output_conf, net_output_coords, net_output_pc], axis=-1)  # (batch_size, n_boxes, n_classes + 5)
                else:
                    # [cl1_conf, cl2_conf, ..., dcx_enc, dcy_enc, w_enc, h_enc]
                    net_output = tf.concat([net_output_conf, net_output_coords], axis=-1)  # (batch_size, n_boxes, n_classes + 4)
        else:
            # [cl1_conf, cl1_dcx_enc, cl1_dcy_enc, cl1_w_enc, cl1_h_enc, cl1_pc, cl2...]
            net_output = net_output_conf  # (batch_size, n_boxes, n_classes)
    ...
    def net_on_every_size(self, inputs_all_sizes, labels, filenames):
        ...
        for i in range(len(self.grid_levels)):
            ...
            net, common_representation = self.net(inputs_all_sizes[i])
            ...
            output_flat = tf.reshape(net, [-1, grid.n_boxes, self.get_last_layer_n_channels()], name='output_flat')
    ...
    def compare_crs(self, cr1, cr2, reuse=tf.AUTO_REUSE):
        # cr1, cr2: (batch_size, lcr)
        with tf.variable_scope('cr_comparison'):
            subtraction = tf.abs(cr1 - cr2)  # (batch_size, lcr)
            fc1 = tf.layers.dense(subtraction, self.opts.lcr, activation=tf.nn.relu, reuse=reuse, name='fc1')
            comparison = tf.layers.dense(fc1, 2, activation=None, use_bias=False, reuse=reuse, name='fc2')
        return comparison
    ...
    def make(self, inputs, labels, filenames):
        ...
        net_output_conf, net_output_coords, net_output_pc = self.split_net_output_tf(net_output)
    ...
    def remove_repeated_predictions(self, net_output_conf, net_output_coords, net_output_pc, comparison_pred, comp_conf1_wins):
        ...
        predictions = self.put_together_net_output(net_output_conf_mod, net_output_coords, net_output_pc)
        print('Operations to remove repeated predictions ready.')
        return predictions

    # def remove_repeated_predictions(self, net_output_conf, net_output_coords, net_output_pc, all_comparisons):
    #     # net_output: (batch_size, nboxes, ?)
    #     # net_output_conf: (batch_size, nboxes, nclasses)
    #     # all_comparisons: list of elements (batch_size, nboxes, 2)
    #     print('Setting operations to remove repeated predictions...')
    #     batch_size = tf.shape(net_output_conf)[0]
    #     softmax = tf.nn.softmax(net_output_conf, axis=-1)  # (batch_size, nboxes, nclasses)
    #     background_conf = tf.concat([tf.zeros(shape=(batch_size, self.n_boxes, self.nclasses - 1), dtype=tf.float32),
    #                                  tf.ones(shape=(batch_size, self.n_boxes, 1), dtype=tf.float32)], axis=-1)
    #     confidences = tf.reduce_max(softmax, axis=-1)  # (batch_size, nboxes)
    #     all_masks = []
    #     for i in range(self.n_boxes):
    #         this_mask = tf.zeros((batch_size), tf.bool)
    #         for comp_idx in range(len(all_comparisons)):
    #             indices = self.comparisons_map[comp_idx]
    #             if i in indices:
    #                 comparison = all_comparisons[comp_idx]  # (batch_size, 2)
    #                 same_prediction = comparison[:, 0] > comparison[:, 1]  # (batch_size)
    #                 conf1 = confidences[:, indices[0]]  # (batch_size)
    #                 conf2 = confidences[:, indices[1]]  # (batch_size)
    #                 conf1_wins = tf.greater(conf1, conf2)  # (batch_size)
    #                 supress_1 = tf.logical_and(same_prediction, tf.logical_not(conf1_wins))  # (batch_size)
    #                 this_mask = tf.logical_or(this_mask, supress_1)  # (batch_size)
    ...
    def get_selected_coords_np(self, net_output_conf, net_output_coords):
        # net_output_conf: (..., nclasses)
        ...
        if self.opts.box_per_class:
            ...
            selected_dcx_enc = np.take_along_axis(net_output_coords[..., :self.nclasses], indices_exp, axis=-1)  # (..., 1)
            selected_dcy_enc = np.take_along_axis(net_output_coords[..., self.nclasses:(2*self.nclasses)], indices_exp, axis=-1)  # (..., 1)
            selected_w_enc = np.take_along_axis(net_output_coords[..., (2*self.nclasses):(3*self.nclasses)], indices_exp, axis=-1)  # (..., 1)
            selected_h_enc = np.take_along_axis(net_output_coords[..., (3*self.nclasses):], indices_exp, axis=-1)  # (..., 1)
    ...
    def get_selected_coords_np_nobatch(self, net_output_conf_nobatch, net_output_coords_nobatch):
        # net_output_conf: (nboxes, nclasses)
        # net_output_coords: (nboxes, 4 * nclasses)
        # [dcx_enc_cl1, dcx_enc_cl2, ..., dcy_enc_cl1, dcy_enc_cl2, ..., w_enc_cl1, w_enc_cl2, ..., h_enc_cl1, h_enc_cl2, ...]
        indices = np.argmax(net_output_conf_nobatch, axis=-1)  # (nboxes)
        indices_exp = np.expand_dims(indices, axis=-1)  # (nboxes, 1)

        selected_dcx_enc = np.take_along_axis(net_output_coords_nobatch[:, :self.nclasses], indices_exp, axis=-1)  # (nboxes, 1)
        selected_dcy_enc = np.take_along_axis(net_output_coords_nobatch[:, self.nclasses:(2*self.nclasses)], indices_exp, axis=-1)  # (nboxes, 1)
        selected_w_enc = np.take_along_axis(net_output_coords_nobatch[:, (2*self.nclasses):(3*self.nclasses)], indices_exp, axis=-1)  # (nboxes, 1)
        selected_h_enc = np.take_along_axis(net_output_coords_nobatch[:, (3*self.nclasses):], indices_exp, axis=-1)  # (nboxes, 1)

        selected_coords = np.concatenate([selected_dcx_enc, selected_dcy_enc, selected_w_enc, selected_h_enc], axis=-1)  # (nboxes, 4)
    ...
    # This is used in case we compute a set of coordinates per class.
    def get_selected_coords_tf(self, indices_mesh, net_output_coords):
        # net_output_coords: (batch_size, nboxes, 4 * nclasses)
        # [dcx_enc_cl1, dcx_enc_cl2, ..., dcy_enc_cl1, dcy_enc_cl2, ..., w_enc_cl1, w_enc_cl2, ..., h_enc_cl1, h_enc_cl2, ...]
        selected_dcx_enc = tf.gather_nd(net_output_coords[:, :, :self.nclasses], indices_mesh)  # (batch_size, nboxes)
        selected_dcy_enc = tf.gather_nd(net_output_coords[:, :, self.nclasses:(2*self.nclasses)], indices_mesh)  # (batch_size, nboxes)
        selected_w_enc = tf.gather_nd(net_output_coords[:, :, (2*self.nclasses):(3*self.nclasses)], indices_mesh)  # (batch_size, nboxes)
        selected_h_enc = tf.gather_nd(net_output_coords[:, :, (3*self.nclasses):], indices_mesh)  # (batch_size, nboxes)
        selected_coords = tf.stack([selected_dcx_enc, selected_dcy_enc, selected_w_enc, selected_h_enc], axis=-1)  # (batch_size, nboxes, 4)
        print('selected_coords: ' + str(selected_coords))
        return selected_coords

    # This is used in case we compute a set of coordinates per class.
    def get_selected_pc_tf(self, indices_mesh, net_output_pc):
        # net_output_pc: (batch_size, nboxes, nclasses)
        # [dcx_enc_cl1, dcx_enc_cl2, ..., dcy_enc_cl1, dcy_enc_cl2, ..., w_enc_cl1, w_enc_cl2, ..., h_enc_cl1, h_enc_cl2, ...]
        selected_pc = tf.gather_nd(net_output_pc, indices_mesh)  # (batch_size, nboxes)
        print('selected_pc: ' + str(selected_pc))
        return selected_pc
    ...
    def compute_localization_and_pc_losses(self, mask_match, labels_enc, net_output_conf, net_output_coords, net_output_pc, n_positives, zeros):
        ...
        if self.opts.box_per_class:
            ...
            selected_coords = self.get_selected_coords_tf(indices_mesh, net_output_coords)  # (batch_size, nboxes, 4)

            localization_loss = smooth_L1_loss(gt_coords, selected_coords)  # (batch_size, nboxes)
            # print('localization_loss = ' + str(localization_loss))

            if self.opts.predict_pc:
                # net_output_pc: (batch_size, nboxes, nclasses)
                selected_pc = self.get_selected_pc_tf(indices_mesh, net_output_pc)  # (batch_size, nboxes)
                pc_loss = smooth_L1_loss(gt_pc_ass, selected_pc, False)  # (batch_size, nboxes)
        else:
            # pred_cords: (batch_size, nboxes, 4)
            localization_loss = smooth_L1_loss(gt_coords, net_output_coords)  # (batch_size, nboxes)
            # print('localization_loss = ' + str(localization_loss))

            if self.opts.predict_pc:
                # net_output_pc: (batch_size, nboxes)
                pc_loss = smooth_L1_loss(gt_pc_ass, net_output_pc, False)  # (batch_size, nboxes)
    ...
    # def comparison_loss(self, all_comparisons, mask_match, nearest_valid_gt_idx, n_positives):
    #     # all_comparisons: list of tensors (batch_size, 2)
    #     # mask_match: (batch_size, nboxes)
    #     # nearest_valid_gt_idx: (batch_size, nboxes)
    #     batch_size = tf.shape(mask_match)[0]
    #     comp_loss = tf.zeros(shape=(batch_size), dtype=tf.float32)
    #     for comp_idx in range(len(all_comparisons)):
    #         comparison = all_comparisons[comp_idx]  # (batch_size, 2)
    #         indices = self.comparisons_map[comp_idx]
    #         same_gt = tf.less(tf.abs(nearest_valid_gt_idx[:, indices[0]] - nearest_valid_gt_idx[:, indices[1]]), 0.5)  # (batch_size), bool
    #         same_gt_int = tf.cast(same_gt, tf.int32)  # (batch_size) int32
    #         loss_this_pair = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=same_gt_int, logits=comparison, name='loss_orig')
    #         comp_loss = tf.add(comp_loss, loss_this_pair)  # (batch_size)
    #     comp_loss_scaled = tf.divide(comp_loss, tf.constant(len(all_comparisons), dtype=np.float32, shape=()), name='comp_loss_scaled')
    #     comp_loss = tf.reduce_mean(comp_loss_scaled, name='comp_loss')
    #     comp_loss = comp_loss * self.opts.comp_loss_factor
    #     return comp_loss
    ...
    def apply_padding_to_gt(self, gt_boxes):
        padded_gt_boxes = []
        for box in gt_boxes:
            padded_gt_boxes.append(box)
        for box in padded_gt_boxes:
            box.apply_padding(self.max_pad_rel)
        return padded_gt_boxes

    def apply_padding_to_box(self, box):
        new_box = copy.deepcopy(box)
        new_box.apply_padding(self.max_pad_rel)
        return new_box
    ...
    def coords_pad2orig(self, coords_pad):
        coords_orig = np.zeros(shape=coords_pad.shape, dtype=np.float32)
        coords_orig[..., :2] = (coords_pad[..., :2] - self.max_pad_rel) / (1 - 2 * self.max_pad_rel)
        coords_orig[..., 2:] = coords_pad[..., 2:] / (1 - 2 * self.max_pad_rel)
        return coords_orig
    ...
    def decode_preds(self, net_output_nobatch, th_conf):
        ...
        net_output_conf, net_output_coords, net_output_pc = self.split_net_output_np(net_output_nobatch)
        # net_output_conf: (nboxes, nclasses)
        # net_output_coords: (nboxes, ?)
        # net_output_pc: (nboxes, ?)
        if self.opts.box_per_class:
            # net_output_coords: (nboxes, 4 * nclasses)
            selected_coords = self.get_selected_coords_np_nobatch(net_output_conf, net_output_coords)  # (nboxes, 4)
    ...
    def decode_preds_as_anchor(self, encoded_preds, th_conf):
        # Inputs:
        #     encoded_preds: numpy array of dimension nboxes x (4 + nclasses).
        # Outputs:
        #     predictions: List of PredictedBox objects, with relative coordinates (between 0 and 1)
        #
        # We don't consider the batch dimension here.
        class_ids = np.argmax(encoded_preds[:, :(self.nclasses-1)], axis=1)  # (nboxes)
        predictions = []
        for idx in range(self.n_boxes):
            if encoded_preds[idx, class_ids[idx]] > encoded_preds[idx, self.background_id]:
                xmin = self.anchors_coordinates[idx, 0]
                ymin = self.anchors_coordinates[idx, 1]
                width = self.anchors_coordinates[idx, 2] - self.anchors_coordinates[idx, 0]
                height = self.anchors_coordinates[idx, 3] - self.anchors_coordinates[idx, 1]
                gtbox = PredictedBox([xmin, ymin, width, height], class_ids[idx], encoded_preds[idx, class_ids[idx]])
                predictions.append(gtbox)
        # Convert boxes coordinates from padded representation to the original one, and clip values to (0, 1):
        predictions = self.remove_padding_from_gt(predictions, clip=True)
        return predictions
    ...
    def encode_boxes(self, coords_raw):
        ...
        if self.opts.encoding_method == 'basic_1':
            ...
            w_enc = logit((w_rel - self.opts.enc_wh_b) / self.opts.enc_wh_a)
            h_enc = logit((h_rel - self.opts.enc_wh_b) / self.opts.enc_wh_a)
    ...
    def decode_boxes(self, coords_enc):
        ...
        if self.opts.encoding_method == 'basic_1':
            ...
            w_rel = np.clip(sigmoid(w_enc) * self.opts.enc_wh_a + self.opts.enc_wh_b, 1.0 / self.receptive_field_size, 1.0)
            h_rel = np.clip(sigmoid(h_enc) * self.opts.enc_wh_a + self.opts.enc_wh_b, 1.0 / self.receptive_field_size, 1.0)
        elif self.opts.encoding_method == 'ssd':
    ...
    def decode_boxes_w_batch_size(self, coords_enc):
        ...
        if self.opts.encoding_method == 'basic_1':
            ...
            w_rel = np.clip(sigmoid(w_enc) * self.opts.enc_wh_a + self.opts.enc_wh_b, 1.0 / self.receptive_field_size, 1.0)  # (batch_size, nboxes)
            h_rel = np.clip(sigmoid(h_enc) * self.opts.enc_wh_a + self.opts.enc_wh_b, 1.0 / self.receptive_field_size, 1.0)  # (batch_size, nboxes)
        elif self.opts.encoding_method == 'ssd':
    ...
    def compute_iou_metric(self, net_output, labels_enc):
        ...
        net_output_conf, net_output_coords, net_output_pc = self.split_net_output_np(net_output)
        selected_coords = self.get_selected_coords_np(net_output_conf, net_output_coords)  # (batch_size, nboxes, 4)
...
def l2_normalization(inputs):
    with tf.variable_scope('L2Normalization'):
        # inputs = tf.Print(inputs, [tf.reduce_min(inputs), tf.reduce_mean(inputs), tf.reduce_max(inputs)], 'before L2 norm')
        inputs_shape = inputs.get_shape()
        nchannels = inputs_shape[3]
        # Normalize along spatial dimensions.
        outputs = tf.nn.l2_normalize(inputs, axis=3, epsilon=1e-12)
        # Additional scaling.
        initial_value = tf.ones(nchannels)
        scale = tf.Variable(initial_value=initial_value, trainable=True, name='scale')
        # outputs = tf.Print(outputs, [tf.reduce_min(outputs), tf.reduce_mean(outputs), tf.reduce_max(outputs)], 'inside L2 norm ')
        outputs = tf.multiply(outputs, scale)
        # outputs = tf.Print(outputs, [tf.reduce_min(outputs), tf.reduce_mean(outputs), tf.reduce_max(outputs)], 'after L2 norm ')
        return outputs
